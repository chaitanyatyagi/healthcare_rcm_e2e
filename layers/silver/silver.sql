...
----------------------------------------------------------------------------------------------------------------------------
----------------  OPTIMIZE QUERY FOR SCD2 TABLES TO REDUCE THE STORAGE COSTS AND IMPROVE THE PERFORMANCE -------------------
----------------------------------------------------------------------------------------------------------------------------

-- STEP 1 — Build staging table (stage) with hash & quarantine
CREATE OR REPLACE TABLE `avd-databricks-demo.silver_dataset.stage_patients` AS
...
SELECT
    ...
    -- quarantine logic
    CASE 
        WHEN SRC_PatientID IS NULL OR DOB IS NULL OR FirstName IS NULL OR LOWER(FirstName) = 'null'
        THEN TRUE ELSE FALSE
    END AS is_quarantined,

    -- record hash for change detection
    TO_HEX(SHA256(CONCAT(
        COALESCE(SRC_PatientID, ''), '|',
        COALESCE(FirstName, ''), '|',
        COALESCE(LastName, ''), '|',
        COALESCE(MiddleName, ''), '|',
        COALESCE(SSN, ''), '|',
...
-- STEP 2 — UPDATE existing records (close old versions)
UPDATE `avd-databricks-demo.silver_dataset.patients` AS t
SET 
  is_current = FALSE,
...
WHERE t.is_current = TRUE
  AND EXISTS (
        SELECT 1
        FROM `avd-databricks-demo.silver_dataset.stage_patients` AS s
        WHERE s.Patient_Key = t.Patient_Key
          AND s.record_hash <> t.record_hash
  );

-- STEP 3 — INSERT new active records (new + changed)
INSERT INTO `avd-databricks-demo.silver_dataset.patients` (
    Patient_Key,
    SRC_PatientID,
    FirstName,
    LastName,
...
SELECT
    ...
    s.record_hash
FROM `avd-databricks-demo.silver_dataset.stage_patients` AS s
LEFT JOIN `avd-databricks-demo.silver_dataset.patients` AS t
ON t.Patient_Key = s.Patient_Key
AND t.is_current = TRUE
WHERE t.Patient_Key IS NULL       -- brand new record
   OR t.record_hash <> s.record_hash; -- updated record
